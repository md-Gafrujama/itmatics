...
- Never use em dashes (${"\u2014"}) or en dashes (${"\u2013"}). Use only the short hyphen "-" (with spaces when needed), or rewrite as two sentences.
- Example good: "AI, cloud, and security - written for IT leaders." Example bad: any long dash character.`;

const ORIGINALITY_RULES = `ORIGINALITY / PLAGIARISM (required):
- Write 100% original ITmatics News prose. Do NOT copy, closely paraphrase, or paste wording from source articles, press releases, or vendor blogs.
- Facts, figures, dates, and named entities may be used; the sentences and structure must be yours.
- Attribute claims to outlets or organizations by name ("according to Reuters", "Gartner reported") - never lift their sentences.
- No verbatim quotes longer than one short clause unless clearly marked as a quote and attributed.
- Titles, dek, lede, sections, takeaways, and meta fields must all be original.`;

const client = new Anthropic({
  // Research + write can take several minutes with web_search tools.
  timeout: 10 * 60 * 1000,
});
...
const ArticleSchema = z.object({
  ...
  meta_title: z
    .string()
    .describe(
      "Browser/SEO <title>. Include focus keyword near the start. End with ' | ITmatics News'. MAXIMUM 60 characters including the brand suffix. Shorter is fine.",
    ),
  meta_description: z
    .string()
    .describe(
      "SEO meta description AND Open Graph description. MAXIMUM 160 characters. Shorter is fine. Include focus keyword once, a concrete hook, and a reason for IT leaders to click. No quotes.",
    ),
  og_title: z
    .string()
    .describe(
  ...
  geo_summary: z
    .string()
    .describe(
      "Generative-engine summary: 2-3 neutral sentences a model can cite. Include the key fact and who it affects (IT leaders). MAXIMUM 400 characters. Shorter is fine.",
    ),
...
/**
 * Step 1: Web-search trending enterprise IT coverage for ONE navbar topic,
...
export async function researchTopic(
  ...
  const avoidList =
    recentTitles.length > 0
      ? `Do NOT repeat or closely rehash any of these already-published ITmatics News titles:\n${recentTitles.map((t) => `- ${t}`).join("\n")}`
      : "No prior ITmatics News articles yet.";
  ...
  const messages: Anthropic.MessageParam[] = [
    {
      role: "user",
      content: `You are the research desk for ITmatics News, an editorial site for IT leaders and enterprise IT teams.
...
YOUR JOB:
1. Use web search to scan DIFFERENT reputable sites for what is trending / newly reported RIGHT NOW in "${topic.name}" for IT leaders, CIOs, and enterprise IT teams.
2. Search across multiple outlets - e.g. The Register, InfoWorld, CIO.com, Dark Reading, TechCrunch Enterprise, Reuters, WSJ, Bloomberg Technology, Gartner, Forrester, IDC, NIST, CISA, and major cloud/security vendor research - not just one domain.
3. Prefer angles that appeared in the last 7-14 days (or are clearly escalating now).
4. Pick ONE concrete, newsworthy story angle that ITmatics News can cover uniquely for enterprise IT leaders.
5. The angle MUST clearly belong under "${topic.name}" - not a different navbar topic.
6. Capture facts and source names only - never paste source article wording into the brief.

Suggested search patterns (run several):
- "${topic.name} enterprise IT news"
- "trending ${topic.name} CIO"
- "${topic.name} IT leaders 2026" (or current year)
...
IT IMPLICATION: why CIOs, CISOs, and enterprise IT leaders care
...
export async function researchTitle(
  ...
  const avoidList =
    recentTitles.length > 0
      ? `Do NOT copy or closely rehash any of these already-published ITmatics News titles:\n${recentTitles.map((t) => `- ${t}`).join("\n")}`
      : "No prior ITmatics News articles yet.";
...
2. Search The Register, InfoWorld, CIO.com, Dark Reading, TechCrunch Enterprise, Reuters, WSJ, Bloomberg Technology, Gartner, Forrester, NIST, and CISA.
...
5. Build a brief the writer can turn into a full SEO article WHILE KEEPING the assigned headline. Capture facts only - do not paste source prose into the brief.
...
Build slug, meta_title, og_title, focus_keyword, and the body around it.`
  ...
  const response = await client.messages.parse({
    ...
    messages: [
      {
        role: "user",
        content: `Using the research brief below, write a full article for ITmatics News (publication for CIOs, CISOs, and enterprise IT leaders). Tone: clear, editorial, non-hype - like CIO.com, The Register, or InfoWorld. Concrete and specific.
...
SEO RULES (required):
${titleRule}
2. meta_title MAXIMUM 60 characters (including " | ITmatics News"). Shorter is fine.
3. meta_description MAXIMUM 160 characters. Shorter is fine. Hook + keyword + IT leader benefit (also used as OG description).
