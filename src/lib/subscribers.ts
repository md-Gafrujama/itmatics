...
export async function unsubscribeByToken(
  ...
  if (cleanReason) payload.unsubscribe_reason = cleanReason.slice(0, 280);
  ...
  // Older DBs may not have unsubscribe_reason yet
  if (error && payload.unsubscribe_reason) {
    const retry = await admin
      .from("subscribers")
      .update({ unsubscribed_at: payload.unsubscribed_at })
      .eq("unsubscribe_token", trimmed)
      .select("id")
      .maybeSingle();
    data = retry.data;
...
export async function unsubscribeByEmail(
  ...
  if (!existing) {
    return {
      ok: false,
      message: "We could not find that email on The Download list.",
    };
  }
  ...
  return { ok: true, message: "You have been unsubscribed from The Download." };
}
...
async function notifySubscribersOfArticleInner(articleId: string) {
  ...
  const subject = `New on ITmatics News: ${article.title}`;
  const from = mailFrom();
  ...
  for (const recipient of recipients) {
    ...
    try {
      await mailer.sendMail({
        ...
        subject,
        text: [
          `A new ITmatics News story${topicLabel} is live.`,
          "",
          article.title,
          article.dek,
          "",
...
<body style="margin:0;padding:24px;background:#f4f1ea;font-family:Georgia,serif;color:#111;">
  <div style="max-width:560px;margin:0 auto;background:#fff;padding:32px;border:1px solid #e7e2d6;">
    <p style="margin:0 0 18px;font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#6b7280;">ITmatics News</p>
    ${topic}
    ...
    <p style="margin:0;font-size:12px;color:#6b7280;">
      You received this because you subscribed to related ITmatics News coverage.
